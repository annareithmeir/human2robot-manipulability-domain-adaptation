...
% intial joint positions franka 1
q_init1=[ 1.3875 1.309 -1.5708 -0.61087 0 2.5307 1.309 ];
q_init2=[ -1.405 1.309 1.5708 -0.61087 0 2.5307 -1.309 ];
qd_init1=[ 0 0 0 0 0 0 0 ];
qd_init2=[ 0 0 0 0 0 0 0 ];

dq_init_1 = fep1.fkm(q_init1);
dq_init_2 = fep2.fkm(q_init2);
...
dqrd = panda_bimanual.relative_pose(q); % relative pose inbetween EEs, q=[q1;q2]
...
Jgeom=geomJ(pose_abs_j, pose_abs); % 6x14
...
%goal_pos = [0.7   -0.0    0.6]; % 'floor_parallel' dataset
goal_pos = [0.0   0.0    1]; % 'reach_up' dataset
p_goal_1 = goal_pos';
robot_vel = [0.05, 0, 0];
...
tt=(0:delta:timesteps*delta-delta); % 500 timesteps (1:0.01:4.99)
manips=zeros(timesteps, 3*14+1);
joints=zeros(timesteps, 14+1);
manips(:,1)=tt';
joints(:,1)=tt';

q_desired=[1.86699435985516;1.10511006712162;-1.48292372566879;-2.08171923374451;0.169647374402207;3.78561692548130;0.820506929977718;-1.90747180469972;1.42365126254395;1.66872090523647;-1.67660553883418;0.114321875825372;2.86888730064485;-1.36809996795840];
...
while jj <= size(t,2)
    ...
    manips(jj, 2:43) = reshape(jacob3, 1, 3*14);
