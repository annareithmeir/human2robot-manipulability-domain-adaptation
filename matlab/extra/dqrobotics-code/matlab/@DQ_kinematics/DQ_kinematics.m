...
classdef DQ_kinematics < handle
    ...
    methods
        ...
        % Return the Twist Jacobian (6 x N)
        function J = jacobianTwist(obj,theta)
            % J = jacobianTwist(theta) returns the Twist DQ Jacobian, where
            % theta is the vector of joint variables
            q_effector = obj.raw_fkm(theta);
            
            q = DQ(1);
            J= zeros(6,obj.links-obj.n_dummy);
            ith=0;
            ...
            for i = 0:obj.links-1
                    ...
                    if ~obj.dummy(i+1)
                        q = q*obj.dh2dq(theta(ith+1),i+1);
                        % Part removed (that makes the DQ Jacobian)
                        % j = z * q_effector; (part removed!)
                        % Part added: take the base's displacement into account
                        j = .5*obj.base*z*obj.base';
                        J(:,ith+1) = [j.q(2:4); j.q(6:8)];
                        ith = ith+1;                    
                    else
                        q = q*obj.dh2dq(0,i+1); %Dummy joints don't contribute to the Jacobian
                    end
            end
            % Part removed: Using only Twist Jacobian 
            % J = hamiplus8(obj.base)*haminus8(obj.effector)*J; %Takes the base's displacement into account
        end        
