...
void GMM_SPD::MStep() {
    ...
    for (int k = 0; k < this->m_k; k++) {
        ...
        for (int n = 0; n < this->m_maxIterM; n++) {
            ...
            logmapvec = LogmapVec(this->m_data.transpose().bottomRows(3),
                                  this->m_muMan.block(1, k, 3, 1)); //row vector
            for (int v = 0; v < logmapvec.size(); v++) {
...
//        }
//        this->m_priors.push_back(collected.size());
//        MatrixXd collectedMatrix, collectedMatrixFull;
//        if(dimensions==2){
//            collectedMatrix = MatrixXd(collected.size(), 3);
//            collectedMatrixFull = MatrixXd(collected.size(), 4); // with time
//        }
//        else{
//            collectedMatrix = MatrixXd(collected.size(), 6);
//            collectedMatrixFull = MatrixXd(collected.size(), 7); // with time
//        }
//
//        for (int l = 0; l < collected.size(); l++) {
//            if(dimensions==2) collectedMatrix.block(l, 0, 1, 3) = data.row(collected[l]).rightCols(3); // collected matrices checked!
//            else collectedMatrix.block(l, 0, 1, 6) = data.row(collected[l]).rightCols(6); // collected matrices checked!
//            collectedMatrixFull.row(l) = data.row(collected[l]);
//        }
//
//        //TODO Find out why small numerical errors to MATLAB code here (in range 1e-03)
...
//            vector<MatrixXd> dataTangent = LogmapVec(collectedMatrix.transpose(),
//                                                     this->m_muMan.col(i).bottomRows(3)); // cut off t data
//            dataTangentMatrix=MatrixXd(4, dataTangent.size());
//            dataTangentMatrix.setZero();
//            for (int i = 0; i < dataTangent.size(); i++) {
//                dataTangentMatrix(0, i) = data(collected[i], 0);
//                dataTangentMatrix.block(1, i, 3, 1) = dataTangent[i].transpose(); //to matrix as in matlab code, with t row
...
//        }
//        else{
//            this->m_muMan.block(1,i,6,1) = Symmat2Vec(SPDMean(Vec2Symmat(collectedMatrix.transpose()), 10)).transpose();
//            // DataTangent checked!
//            vector<MatrixXd> dataTangent = LogmapVec(collectedMatrix.transpose(),
//                                                     this->m_muMan.col(i).bottomRows(6)); // cut off t data
//            dataTangentMatrix=MatrixXd(7, dataTangent.size());
//            dataTangentMatrix.setZero();
//            for (int i = 0; i < dataTangent.size(); i++) {
//                dataTangentMatrix(0, i) = data(collected[i], 0);
//                dataTangentMatrix.block(1, i, 6, 1) = dataTangent[i].transpose(); //to matrix as in matlab code, with t row
//            }
//        }
//
//        // Cov calculation checked!
...
//    this->m_L.setZero();
//    this->m_gamma = MatrixXd(this->m_k, data.cols());
//    this->m_H = MatrixXd(this->m_k, data.cols()); // activation weights
//}
//
////Checked! (numerically slightly different)
//VectorXd GMM_SPD::GaussPDF(const MatrixXd& data, const MatrixXd& mu, const MatrixXd& sig) {
...
//    pdf = (tmp.array() * dataCentered.array()).matrix().rowwise().sum();
//    pdf = (-0.5 * pdf).array().exp() /
//          sqrt(pow(2 * M_PI, this->m_dimVar) * abs(sig.determinant()) + numeric_limits<double>::min()); //TODO dimvar+1?
//    return pdf;
//}
//
//double GMM_SPD::GaussPDF(double data, double mu, double sig) {
...
//        }
//        else{
//            logmapvec = LogmapVec(this->m_data.transpose().bottomRows(6),
//                                                   this->m_muMan.block(1, k, 6, 1)); //row vector
//            for (int v = 0; v < logmapvec.size(); v++) {
//                xts.block(1, v, 6, 1) = logmapvec[v].transpose();
//            }
//        }
//
//        this->m_xts.push_back(xts);
...
//                                      this->m_muMan.block(1, k, 6, 1)); //row vector
//                for (int v = 0; v < logmapvec.size(); v++) {
//                    uTmp.block(1, v, 6, 1) = logmapvec[v].transpose();
//                }
//            }
//
//            uTmpTot = (uTmp.array() *
...
//                                                        this->m_muMan.block(1, k, 3, 1))[0].transpose();
//            else this->m_muMan.block(1, k, 6, 1) = ExpmapVec(uTmpTot.bottomRows(6),this->m_muMan.block(1, k, 6, 1))[0].transpose();
//        }
//        this->m_sigma[k] = (uTmp * this->m_H.row(k).asDiagonal() * uTmp.transpose()) +
//                (tmpId * this->m_regTerm);
//
...
//
//
//    MatrixXd uHat, xHat, Ac,uoutTmp,vMatTmp,pvKTmp, expSigmaT;
//    if(dimensions==2){
...
//                deb(k);
//                deb(this->m_muMan);
//                if(dimensions==2) S1 = Vec2Symmat(this->m_muMan.block(1, k, 3, 1));
//                else S1 = Vec2Symmat(this->m_muMan.block(1, k, 6, 1));
//                S2 = Vec2Symmat(xHat.col(t));
//                Ac(0, 0) = 1;
//                if(dimensions==2) Ac.bottomRightCorner(2, 2) = ParallelTransport(S1[0], S2[0]);
//                else Ac.bottomRightCorner(3, 3) = ParallelTransport(S1[0], S2[0]);
//                deb(k);
//
//                pvMatK.clear();
//                vMatK.clear();
...
//                    vMatTmp.setZero();
//                    vMatTmp(0, 0) = V[k](0, j);
//                    if(dimensions==2) vMatTmp.bottomRightCorner(2, 2) = Vec2Symmat(
//                            V[k].block(1, j, 3, 1))[0]; //Vec2Symmat only one matrix here
//                    else vMatTmp.bottomRightCorner(3, 3) = Vec2Symmat(
...
//                    pvKTmp(0, j) = pvMatK[j](0, 0);
//                    if(dimensions==2) vMatTmp.bottomRightCorner(2, 2) = Vec2Symmat(
//                                V[k].block(1, j, 3, 1))[0]; //Vec2Symmat only one matrix here pvKTmp.col(j).bottomRows(3) = Symmat2Vec(pvMatK[j].block(1, 1, 2, 2)).transpose();
//                    else vMatTmp.bottomRightCorner(3, 3) = Vec2Symmat(
//                                V[k].block(1, j, 6, 1))[0]; //Vec2Symmat only one matrix here pvKTmp.col(j).bottomRows(3) = Symmat2Vec(pvMatK[j].block(1, 1, 2, 2)).transpose();
//                }
//
//                pvMat.push_back(pvMatK);
//                vMat.push_back(vMatK);
